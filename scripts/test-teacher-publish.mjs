...
try {
  ...
  await test("reproduces draft insert returning ownership visibility failure", () => denied(`insert into public.lms_courses(id,title,type,instructor_id) values('returning-test','Draft','short','${teacher}') returning *;`));
  await test("teacher can save then read the draft in a separate request", async () => {
    await db.exec(`insert into public.lms_courses(id,title,type,instructor_id) values('separate-test','Draft','long','${teacher}');`);
    const result = await db.query(`select * from public.lms_courses where id='separate-test'`);
    assert.equal(result.rows.length, 1);
    assert.equal(result.rows[0].instructor_id, teacher);
    await db.exec(`delete from public.lms_courses where id='separate-test'`);
  });
  ...
  await test("teacher cannot publish before migration", () => denied(`update public.lms_courses set status='published' where id='free'`));
  await db.exec('reset role');
  const publishMigration = await readFile(new URL('../supabase/migrations/202609220001_teacher_publish.sql', import.meta.url), 'utf8');
  await db.exec(publishMigration);
  await db.exec(publishMigration);
  await as('authenticated', teacher);
  await test('teacher publishes own course', async () => {
    await db.exec(`update public.lms_courses set status='published',published_at=now() where id='free'`);
    assert.equal((await db.query(`select status from public.lms_courses where id='free'`)).rows[0].status, 'published');
  });
  await test('teacher cannot feature own course', () => denied(`update public.lms_courses set featured=true where id='free'`));
  await db.exec('reset role');
  await db.exec(`update public.lms_profiles set role='teacher' where id='${other}'`);
  await as('authenticated', other);
  await test('another teacher cannot publish someone elses course', async () => {
    assert.equal((await db.query(`update public.lms_courses set status='published' where id='paid' returning id`)).rows.length, 0);
  });
  await db.exec('reset role');
  await db.exec(`update public.lms_profiles set role='student' where id='${other}'`);
