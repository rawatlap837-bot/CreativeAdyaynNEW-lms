-- Courses without an active batch are self-paced. Their modules unlock by
-- paid EMI stage instead of classroom dates. Scheduled courses retain the
-- existing batch-calendar behavior.
...
begin
  ...
  if coalesce(array_length(days,1),0)=0 then
    ...
    -- Registration unlocks the first module. Remaining modules are spread
    -- across monthly installments and unlock immediately when their assigned
    -- installment is paid; no batch dates are needed for recorded learning.
    for r in
  ...
  loop
    ...
    insert into public.lms_module_access(
      ...
      case when current_date>=public.nth_session_date(start_on,days,r.n)
        and (p_plan_id is null or installment.status='paid') then now() end,
      now()
    )
    on conflict(student_id,module_id) do update set
      plan_id=excluded.plan_id,
